...
if ($validar->estavazio($nome)) {
    ...
    header("Location: ../../../view/public/home.php?opcao=4.1&adbrd=teidra&eloua={$_SESSION['id_edit']}");
    exit;
}
...
if(isset($_GET['eloua'])){
    ...
    $registo = $banco->select("SELECT foto_perfil FROM funcionario WHERE id_funcionario=$id")->fetch();
    if($foto['name'] != $registro){
    move_uploaded_file($foto['tmp_name'],"../../../view/assets/img/funcionarios/{$foto['name']}");
    }
     $admin = new Admin();
     if($admin -> editarFuncionario("UPDATE `funcionario` SET `nome`='$nome',`data_nasc`='$data',`bi`='$bi',`sexo`='$sexo',`foto_perfil`='{$foto['name']}',`cargo`='$cargo',`salario`='$salario' WHERE id_funcionario = '$id'") && $admin->editarFuncionario("UPDATE `funcionario` f join contactos c on f.id_contacto=c.id_contactos set telemovel = '$telefone', telefone_alt ='$telefone_alt',email ='$email' WHERE id_funcionario='$id'") && $admin->editarFuncionario("UPDATE `funcionario` f join morada m on f.id_morada=m.id_morada set municipio = '$municpio', rua_casa ='$rua',numero_casa ='$n_casa' WHERE id_funcionario='$id'")){
